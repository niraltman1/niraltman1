...
/**
 * Parses fetched law HTML (Hebrew WikiSource "ספר החוקים הפתוח" rendering) into verbatim,
 * per-section slices. Ported from PR #50's parser.ts — the HTML→text + section-splitter
 * logic is unchanged; only its home moved into the build-tool so no runtime package fetches.
 *
 * SAFETY CONTRACT:
