...
function _generateInitialPassword(): string {
  const chars = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%';
  const bytes = randomBytes(16);
  return [...bytes].map((b) => chars[b % chars.length]!).join('');
}

// If no env var is set, a unique random password is generated once per process.
// It is printed to the console on first boot — the operator must note it.
const DEFAULT_ADMIN_PASSWORD = process.env['FACTUM_IL_ADMIN_PASS'] ?? _generateInitialPassword();
const _isAutoGenerated = !process.env['FACTUM_IL_ADMIN_PASS'];
...
export function seedDefaultAdmin(repos: Repos): void {
  ...
  if (count === 0) {
    ...
    if (_isAutoGenerated) {
      console.warn(`\n[Auth] *** FIRST-RUN GENERATED PASSWORD ***`);
      console.warn(`[Auth] Username : ${DEFAULT_ADMIN_USERNAME}`);
      console.warn(`[Auth] Password : ${DEFAULT_ADMIN_PASSWORD}`);
      console.warn(`[Auth] Set FACTUM_IL_ADMIN_PASS env var to use a fixed password.`);
      console.warn(`[Auth] Change the password after first login.\n`);
    } else {
