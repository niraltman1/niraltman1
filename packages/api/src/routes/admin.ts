...
export function adminRouter(repos: Repos, healingService: RagHealingService, dbPath?: string): Router {
  ...
  router.get('/stats', asyncHandler((_req, res) => {
    ...
    ok(res, {
      ...
      tasksOverdue:    one<number>(`SELECT COUNT(*) AS c FROM Tasks WHERE priority = 'critical'`),
      ...
      trafficAlerts:   one<number>(`SELECT COUNT(*) AS c FROM TrafficCases WHERE statute_deadline IS NOT NULL AND statute_deadline <= date('now','+90 days') AND lifecycle_state NOT IN ('closed','statute_lapsed')`),
