 ...
 *
 * Usage:
 *   pnpm ingest-knesset-odata -- [--out <path>] [--embed] [--limit N] [--only <id,id>] [--delay ms] [--domain-batches|--no-domain-batches] [--batch-size N]
 *
 *   --out              Output artifact path or directory (default: assets/legal-corpus/batches/).
 ...
 *   --domain-batches   (default) Write one file per legal domain: batch-criminal.jsonl.gz, etc.
 *   --no-domain-batches  Fall back to --batch-size numeric batching (default size 100).
 *   --batch-size N     Used only with --no-domain-batches: N laws per file. Default: 100.
 */
...
function parseArgs(): RunOptions {
  ...
  let domainBatches = true; // default: domain-batch mode
  ...
  for (let i = 0; i < a.length; i++) {
    ...
    else if (k === '--domain-batches') domainBatches = true;
    else if (k === '--no-domain-batches') domainBatches = false;
  ...
  if (domainBatches) {
    opts.domainBatches = true;
    opts.out = explicitOut ?? DEFAULT_BATCH_OUT;
  } else {
