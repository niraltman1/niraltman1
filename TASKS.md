...
### What to do next — Milestones הבאים
- **M6 Entity-Centric Navigation** (P1, יסודי/גדול — דורש שינוי pipeline): אכלוס `Entities`/
  `EntityRelations` (mig 042, לא בשימוש) דרך legal-ontology בעת חילוץ + `/api/entities` +
  שמות שופט/ביהמ"ש לחיצים. בעל blast-radius גדול יותר (נוגע ב-pipeline) — מומלץ סבב ייעודי.
- **M7 Smart Collections** (P2): אוספים וירטואליים (פילטרים שמורים מעל מסמכים).
- **Legal Workbench**: התכנסות 3-פאנלים (כבר התחילה ב-M5/HearingPrep).
