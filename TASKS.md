...
## בסיס הידע המשפטי — Legal Corpus (verbatim, מבודד לחלוטין) — תשתית (2026-06-02)

לפי הוראת המשתמש: בסיס ידע משפטי ליבה (28 חוקים/תקנות/הנחיות) — ingestion, vectorization,
ואינדוקס גלובלי ל-RAG/חיפוש סמנטי. **בידוד מוחלט בין חוקים** ו**ציטוט verbatim מדויק**.

**Schema (migration 061):**
- `LegalSources` (חוק = שורה עצמאית, `source_key` UNIQUE), `LegalSections`
  (`verbatim_text_he NOT NULL`, FK NOT NULL לחוק יחיד, `UNIQUE(source_id, section_label)`),
  `LegalSectionEmbeddings` (פר-סעיף, נושא `source_id` כדי ש-RAG לעולם לא יערבב חוקים),
  `fts_legal_sections` (FTS5 + טריגרים) לחיפוש מילולי ללא Ollama. בידוד מובנה: סעיפים
  מחוקים שונים לעולם אינם מתמזגים.

**Backend:**
- `packages/database/src/queries/legal-corpus.ts` — `LegalCorpusRepository`:
  `replaceSections` כותב מחדש סעיפי חוק יחיד ב-transaction אטומי (בידוד+אידמפוטנטיות),
  `searchSections` (תוצאות מתויגות-מקור, ניתנות לצמצום לחוק אחד), embeddings פר-סעיף.
  בדיקות: `legal-corpus.test.ts` (5).
- `packages/api/src/modules/legal-corpus/` — `manifest.ts` (28 מקורות, metadata בלבד),
  `parser.ts` (HTML→סעיפים verbatim; fallback ל-section יחיד 'full'; חוזה: לעולם לא ממציא/
  משכתב), `ingest.ts` (fetch/local→parse→upsert מבודד→embed, graceful). בדיקות parser (6).
- `routes/legal-corpus.ts` — `GET /sources`, `/sources/:key`, `/search`. חיווט `legalCorpus`
  ל-Repos. בדיקות route ב-app.test.ts → API 110.
- `scripts/ingest-legal-corpus.ts` — CLI: `--from-dir` (offline), `--embed`, `--only`.

**חסם רשת (תיעוד):** ל-environment יש egress allowlist — `host_not_allowed` על
he.wikisource.org / gov.il / example.com. לכן **ה-ingestion החי לא בוצע מכאן**. כדי להריצו:
להוסיף את `he.wikisource.org` ו-`www.gov.il` ל-allowlist של ה-environment, ואז
`tsx scripts/ingest-legal-corpus.ts` (או `--from-dir` עם קבצים מקומיים). אפס שינוי קוד.
לא הומצא טקסט חוק כלשהו — המסד נשאר ריק עד הזנת מקור אמיתי.
